...
LEGOMEM_NODE_PATH=${LEGOMEM_NODE_PATH:-/dev/shm/legomem_node0}
...
truncate -s "$LEGOMEM_NODE_SIZE" "$LEGOMEM_NODE_PATH"
...
echo "  NUMA backing: ${LEGOMEM_NODE_PATH} (${LEGOMEM_NODE_SIZE})"
...
exec "$QEMU_BINARY" \
    ...
    -object memory-backend-file,id=legomem-node1,share=on,mem-path="$LEGOMEM_NODE_PATH",size="$LEGOMEM_NODE_SIZE" \
